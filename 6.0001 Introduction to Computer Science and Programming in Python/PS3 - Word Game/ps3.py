...
def get_frequency_dict(sequence):
    ...
    freq = {}
    for x in sequence:
        freq[x] = freq.get(x,0) + 1 #Using d.get(key, default) returns the value for key if key is in the dictionary d, else it return default. This way, the method never raises a KeyError.
    return freq
...
def get_word_score(word, n):
    ...
    score2 = max((7 * len(lword) - 3 * (n - len(lword))), 1)
    score = score1 * score2
    return score
...
def is_valid_word(word, hand, word_list):
    ...
    def hand_checker(one_word, one_hand):
        copy_hand = copy.deepcopy(one_hand)
        list_word = list(one_word)
        copy_list_word = list_word[:]
        for letter in copy_list_word:
    ...
    list_word = []
    if "*" in lword:
        list_of_letters = list(lword)
        vowel = list_of_letters.index("*")
        for letter in VOWELS:
            list_of_letters[vowel] = letter
            test_word = "".join(list_of_letters)
            if test_word in word_list:
                checker += 1
    else:
        if lword in word_list:
            if hand_checker(lword, hand) == 0:
                return True
            else:
                return False
    if checker > 0:
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    # BEGIN PSEUDOCODE Pseudocode can directly become the comments.
        ...
        for i in range(replacements):
            ...
            while new_choice:
                ...
                if new_letter != letter:
                    copy_hand[new_letter] = copy_hand.get(new_letter, 0) + 1
                    new_choice = False
    return copy_hand
    ...
    for i in range(number_of_hands):
        if switch == False:
            ...
            if substitute == "yes":
                ...
                if replay == False:
                    again = input("Would you like to replay the hand? ").lower()
                    if again == "yes":
                        replay = True
